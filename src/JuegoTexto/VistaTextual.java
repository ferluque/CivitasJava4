...
class VistaTextual {
    ...
    //Revisar código (es de Israel)
    Respuestas comprar() {
        int opcion = menu("¿Deseas comprar la calle a la que se has llegado? ",
                new ArrayList<>(Arrays.asList("Si", "No")));
        return Respuestas.values()[opcion];
    }

    void gestionar() {
        ArrayList <String> gestiones = new ArrayList<String>();
        for (int i=0; i<GestionesInmobiliarias.values().length; i++)
            gestiones.add(GestionesInmobiliarias.values()[i].toString());
        int opcion = menu("¿Qué gestión inmobiliaria desea realizar?", gestiones);
        iGestion = opcion;
